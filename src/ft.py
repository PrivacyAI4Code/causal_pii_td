...
class EpochSaveCallback(TrainerCallback):
    ...
    def __init__(self, model_args, training_args, tokenizer):
    ...
    def on_epoch_end(self, args, state, control, model=None, **kwargs):
        ...
        if self.model_args.save_every_epoch:
            epoch_output_dir = os.path.join(self.training_args.output_dir, f"{base_model}-{task}-checkpoint-epoch-{int(state.epoch)}")
            os.makedirs(epoch_output_dir, exist_ok=True)
            if self.model_args.use_lora:
                # Save LoRA adapter
                model.save_pretrained(epoch_output_dir)
                self.tokenizer.save_pretrained(epoch_output_dir)
                log_print(f"Saved LoRA checkpoint at epoch {int(state.epoch)} to {epoch_output_dir}")
                if self.training_args.push_to_hub:
                    # model.push_to_hub(base_model + f"-{task}-checkpoint-epoch-{int(state.epoch)}", token=self.model_args.token)
                    # self.tokenizer.push_to_hub(base_model + f"-{task}-checkpoint-epoch-{int(state.epoch)}", token=self.model_args.token)
                    # log_print(f"Pushed LoRA checkpoint at epoch {int(state.epoch)} to hub")
                    repo_name = self.model_args.hub_name or base_model
                    model.push_to_hub(
                        repo_name,
                        folder_in_repo=f"{base_model}-{task}-checkpoint-epoch-{int(state.epoch)}",
                ...
                model.save_pretrained(epoch_output_dir)
                self.tokenizer.save_pretrained(epoch_output_dir)
                log_print(f"Saved full model checkpoint at epoch {int(state.epoch)} to {epoch_output_dir}")
                if self.training_args.push_to_hub:
                    repo_name = self.model_args.hub_name or base_model
                    model.push_to_hub(
                        repo_name,
                    ...
                    log_print(f"Pushed full model checkpoint at epoch {int(state.epoch)} to hub")
...
def main():
    ...
    if model_args.save_every_epoch:
        epoch_save_callback = EpochSaveCallback(model_args, training_args, tokenizer)
