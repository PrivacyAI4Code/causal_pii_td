#!/usr/bin/env python
# https://github.com/huggingface/transformers/blob/main/examples/pytorch/language-modeling/run_fim.py
"""
...
from utils import log_print # log_print is a function that prints with detailed information, could also be defined simply as print()

hf_token = "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"
...
def load_args():
    ...
    parser.add_argument("--validation_file", type=str, required=True, help="path to validation file")
...
def flatten_config(config: dict) -> dict:
    """
    Recursively extract all non-dict values from a nested config dict
    and return them as a flat dictionary.
    """
    flat_dict = {}
...
class EpochSaveCallback(TrainerCallback):
    """Callback to save model after every epoch when save_every_epoch is enabled."""
    ...
    def on_epoch_end(self, args, state, control, model=None, **kwargs):
        hub_name = self.model_args.hub_name
        base_model = self.model_args.model_name_or_path.split("/")[-1]
        task = "fim" if self.data_args.using_fim else "ft"
        ft_type = "lora" if self.model_args.use_lora else "full"
        
        # Handle hub_name properly
        if hub_name:
            exp_name = f"{hub_name}/{self.model_args.experiment_name}"
        else:
            exp_name = self.model_args.experiment_name
            
        version = f"{base_model}-{task}-{ft_type}-epoch-{int(state.epoch)}"
        if self.model_args.save_every_epoch:
            model.push_to_hub(repo_id=exp_name, revision=version, token=hf_token)
            self.tokenizer.push_to_hub(repo_id=exp_name, revision=version, token=hf_token)
            log_print(f"Pushed {ft_type} model checkpoint at epoch {int(state.epoch)} to hub at {exp_name} with version {version}")
        return control
...
class ModelArguments:
    ...
    token: str = field(default=None) # The token to use to access the model.
...
class DataTrainingArguments:
    ...
    # do_test: bool = field(default=False) # Whether get the accuracy of the test set.
    #do_dynamic: bool = field(default=False) # Whether to get the prediction of the dynamic finetuning of dynamic set. 
...
def main():
    ...
    # get all args from config and combine with args, fill the same name parameters in parser
    model_args, data_args, training_args = parser.parse_dict(merged_args, allow_extra_keys=True)

    log_print(f"model_args: {model_args}")
    log_print(f"data_args: {data_args}")
    log_print(f"training_args: {training_args}")

    log_print(f"Process rank: {training_args.local_rank}, device: {training_args.device}, n_gpu: {training_args.n_gpu}, "
        + f"distributed training: {training_args.parallel_mode.value == 'distributed'}, training date type: {model_args.torch_dtype}")
    ...
    # Always initialize numpy random state, regardless of using_fim
    ...
    if model_args.config_name:
        ...
        log_print("You are instantiating a new config instance from scratch.")
        if model_args.config_overrides is not None:
            log_print(f"Overriding config: {model_args.config_overrides}")
            config.update_from_string(model_args.config_overrides)
            log_print(f"New config: {config}")
    ...
    if model_args.model_name_or_path:
        ...
        log_print(f"Training new model from scratch - Total size={n_params / 2**20:.2f}M params")
    ...
    if is_deepspeed_zero3_enabled():
        ...
        with deepspeed.zero.GatheredParameters(original_embeddings.weight, modifier_rank=0):
            ...
            # Convert to float32 for numerical stability in covariance computation
            original_weights_f32 = original_embeddings.weight.data.float()
            mean = original_weights_f32.mean(dim=0)
            n = original_weights_f32.size(0)
            sigma = ((original_weights_f32 - mean).T @ (original_weights_f32 - mean)) / n
            ...
            new_token_embeddings = new_token_embeddings.to(original_embeddings.weight.data.dtype)
    else:
        original_embeddings = model.get_input_embeddings()
        embedding_weights = original_embeddings.weight.data # added by HY
        # Get the pre-expansion embeddings of the model and resize the embedding layer
        model.resize_token_embeddings(len(tokenizer), pad_to_multiple_of=pad_factor)
        embeddings = model.get_input_embeddings()
        ...
        # Convert to float32 for numerical stability in covariance computation
        embedding_weights_f32 = embedding_weights.float()
        mean = embedding_weights_f32.mean(dim=0) # added by HY
        n = embedding_weights_f32.size(0) # added by HY
        sigma = ((embedding_weights_f32 - mean).T @ (embedding_weights_f32 - mean)) / n # added by HY
        ...
        new_token_embeddings = new_token_embeddings.to(embedding_weights.dtype)
    ...
    log_print("Added special tokens to the tokenizer and resized model's embedding layer")

    # Initialize LoRA if enabled
    if model_args.use_lora:
        log_print("Setting up LoRA configuration...")
        ...
        log_print("LoRA configuration applied successfully")
    ...
    def apply_fim(examples):
        ...
        if data_args.using_fim:
            ...
            examples["input_ids"] = examples["input_ids"]
    ...
    with training_args.main_process_first(desc="processing texts together"):
        if not data_args.streaming:
            fim_datasets = tokenized_datasets.map(
                ...
                num_proc=data_args.preprocessing_num_workers,
                load_from_cache_file=not data_args.overwrite_cache,
                desc="Performing FIM transformation" if data_args.using_fim else "Processing data (FIM disabled)",
            )
    ...
    # if training_args.do_dynamic:
    #     if "dynamic" not in tokenized_datasets:
    #         raise ValueError("--do_dynamic requires a dynamic dataset")
    #     dynamic_dataset = lm_datasets["dynamic"]
    ...
    # Prepare callbacks
    callbacks = []
    if model_args.save_every_epoch:
        epoch_save_callback = EpochSaveCallback(model_args, training_args, data_args, tokenizer)
    ...
    kwargs = {"finetuned_from": model_args.model_name_or_path, "tasks": "code completion with fim"}
    if data_args.dataset_name is not None:
